...
GENERAL = ["amenity", "cuisine", "name", "phone"]
...
def add_visible(row, node):
    if "visible" in row.keys(): node["visible"] = row["visible"]
        
def add_created(fields, record, node):
    node["created"] = dict([field, record[field]] for field in fields)
...
def add_pos(row, node):
    if set(["lat", "lon"]) <= set(row.keys()):
        node["pos"] = [float(row["lat"]), float(row["lon"])]
